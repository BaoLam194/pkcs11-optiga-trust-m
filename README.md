...
This example allows the usage of 2 interfaces, either OPENSSL Engine Interface or OPENSSL Provider Interface
...
Note: the PKCS11 provider in this repo only exists in Trixie(stable) package, for Bookworm(stable) you need to enable the [Bookworm-backports](https://wiki.debian.org/Backports) before running the setup_dependencies.sh or build from [source](https://github.com/openssl-projects/pkcs11-provider) and update module in openssl_pkcs11.cnf properly
...
For examples for ECC algorithm through OpenSSL Provider interface:

From root folder :

```console
cd tests/openssl/openssl_provider
chmod +x openssl_pkcs11_prov_ecc.sh
dos2unix openssl_pkcs11_prov_ecc.sh
./openssl_pkcs11_prov_ecc.sh
```
...
```console
cd tests/openssl/openssl_provider
chmod +x openssl_pkcs11_prov_rsa.sh
dos2unix openssl_pkcs11_prov_rsa.sh
./openssl_pkcs11_prov_rsa.sh
```

2. To clean the files generated by the execution of openssl_pkcs11_prov_ecc.sh and openssl_pkcs11_prov_rsa.sh, the script clean.sh can be used :
